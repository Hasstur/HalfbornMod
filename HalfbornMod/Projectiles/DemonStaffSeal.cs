...
{
    public class DemonStaffSeal : ModProjectile
    {
        NPC target;
         ...
         private void UpdateStickyJavelins(NPC target)
         {
             ...
             for (int i = 0; i < Main.maxProjectiles; i++) 
             {
                 ...
                 if (i != projectile.whoAmI 
                     ...
                     && currentProjectile.modProjectile is DemonStaffSeal sealProjectile 
             ...
             if (currentSealIndex >= 1)
             {
                 ...
                 for (int i = 1; i < 1; i++)
                 {
                     ...
                     if (stickingSeals[i].Y < stickingSeals[oldJavelinIndex].Y)
                     {
                         oldJavelinIndex = i; // Remember the index of the removed javelin
                     }
                 }
                 // Remember that the X value in our point array was equal to the index of that javelin, so it's used here to kill it.
                 Main.projectile[stickingSeals[oldJavelinIndex].X].Kill();
             }
         }
